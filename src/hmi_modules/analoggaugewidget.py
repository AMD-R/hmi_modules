...
from PyQt5.QtGui import (QPalette, QConicalGradient, QGradient,
                         QRadialGradient, QFontMetricsF, QFont, QPainter, QPen,
...
class QRoundProgressBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.m_max = 100
        self.m_value = 0
        self.m_nullPosition = QRoundProgressBar.PositionBottom
    ...
    def drawBase(self, p: QPainter, baseRect: QRectF):
        if self.m_barStyle == self.BarStyle.DONUT:
            ...
            p.drawEllipse(baseRect.adjusted(self.m_outlinePenWidth / 2,
    ...
    def drawValue(self, p: QPainter, baseRect: QRectF, value: float,
        ...
        if self.m_barStyle == self.BarStyle.LINE:
            ...
            if value == self.m_max:
                ...
                arcLength = 360 / delta
                p.drawArc(baseRect.adjusted(self.m_outlinePenWidth / 2,
                                            self.m_outlinePenWidth / 2,
                                            -self.m_outlinePenWidth / 2,
                                            -self.m_outlinePenWidth / 2),
    ...
    def valueToText(self, value: float):
        ...
        if self.m_updateFlags == self.UpdateFlags.VALUE:
            textToDraw = textToDraw.replace('%v', str(round(value,
                                                            self.m_decimals)))
        if self.m_updateFlags == self.UpdateFlags.PERCENT:
            procent = (value - self.m_min) / (self.m_max - self.m_min) * 100
            textToDraw = textToDraw.replace('%p', str(round(procent,
                                                            self.m_decimals)))
        if self.m_updateFlags == self.UpdateFlags.MAX:
            textToDraw = textToDraw.replace('%m', str(round(
                self.m_max - self.m_min + 1, self.m_decimals
            )))
        return textToDraw
